...
/**
 * Integration test for Create, Read, Update and Delete operations on JPA Person
 * entities. During the Maven build, it is using a SAP HANA DB running on SAP
 * Cloud Platform accessed via JDBC tunnel. The DB tunnel is opened by the build
 * using the SAP Cloud Platform Maven Plugin goal open-db-tunnel. If executed as
 * plain JUnit test without first opening a DB tunnel, it will use an embedded
 * Derby DB instead.
 */
public class PersonCRUDIntegrationTest {
	...
	private EntityManagerFactory createEntityManagerFactory() {
		...
		// Use SAP HANA DB properties set by SAP Cloud Platform Maven Plugin
		// open-db-tunnel maven goal.
		// If no DB tunnel properties are available, fall back to embedded Derby
		// DB.
	...
	/**
	 * Create a person entity, and make sure that the "AllPersons" query result
	...
	/**
	 * Create a person entity, change an attribute of it and then make sure that
